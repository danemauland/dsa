...
function lucasNumber(n, memo = {0: 2, 1: 1}) {
    ...
    const ans = lucasNumber(n - 1, memo) + lucasNumber(n - 2, memo);
    memo[n] = ans;
...
function fileFinder(directories, targetFile) {
    ...
    //     if (!contained) {
    //         contained = key[0] === "/" ? fileFinder(directories[key], targetFile) : key === targetFile;
    //     }
    // })
...
function pathFinder(directories, targetFile) {
    ...
    // Object.keys(directories).forEach(key => {
    //     if (!pathName) {
    //         if (key[0] === "/" && (pathName = pathFinder(directories[key], targetFile))) {
    ...
    for (let dir in directories) {
        if (dir === targetFile) return '/' + dir;
        const path = pathFinder(directories[dir], targetFile);
